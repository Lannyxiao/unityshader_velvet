...
#ifndef XL_UNITY_GLOBAL_ILLUMINATION_INCLUDED
#define XL_UNITY_GLOBAL_ILLUMINATION_INCLUDED
...
#include "XL_UnityImageBasedLighting.cginc"
#include "XL_UnityStandardUtils.cginc"
...
{
    // Let's try to make realtime shadows woXL on a surface, which already contains
    ...
    // 3) Pick original lightmap value, if it is the daXLest one.
    ...
    // 2) Allows user to define overall ambient of the scene and control situation when realtime shadow becomes too daXL.
    half3 realtimeShadow = max(subtractedLightmap, shadowColor);
    realtimeShadow = lerp(realtimeShadow, lightmap, shadowStrength);

    // 3) Pick daXLest color
